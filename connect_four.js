...
const EMPTY = '⚪'; // White for empty
const PLAYER = '🟡'; // Black for player
const OPPONENT = '🔴'; // Red for opponent
const ROWS = 6;
const COLS = 7;
...
function printBoard() {
    console.log('  0   1   2   3   4   5   6'); // Column labels
    for (let row = 0; row < ROWS; row++) {
        let rowStr = '|';
        for (let col = 0; col < COLS; col++) {
            rowStr += ' ' + board[row][col] + ' |'; // Row cells with borders
        }
        console.log(rowStr);
...
// Heuristic for easy difficulty
function heuristicEasy(board) {
    let score = 0;
    for (let row = 0; row < ROWS; row++) {
        for (let col = 0; col < COLS; col++) {
            if (board[row][col] === OPPONENT) {
                score += 1; // AI pieces add to score
            } 
            else if (board[row][col] === PLAYER) {
                score -= 1; // Player pieces subtract from score
            }
        }
    }
    return score;
...
function aStarSearch(board, player) {
    ...
    while (openList.length > 0) {
        ...
        for (let successor of successors) {
            ...
            successor.h = heuristicEasy(successor.board);
...
function startGame() {
    rl.question('Choose difficulty (easy/hard): ', (difficulty) => {
        if (difficulty !== 'easy' && difficulty !== 'hard') {
            console.log('Invalid choice. Please select "easy" or "hard".');
            startGame();
            return;
        }
...
function playGame(difficulty) {
    rl.question('Your move! Choose a column (0-6): ', (column) => {
        ...
        if (column >= 0 && column < COLS) {
            ...
            const aiMove = bestMove(board, difficulty);
            placePiece(aiMove, OPPONENT);
